...
```{r fig5-9, warning=F, fig.cap="2015년 일자별 구글 주가"}
...
table5_1 <- 
  tibble(
    Percentage = c(seq(50, 95, 5), 96, 97, 98, 99),
    Multiplier = c(0.67, 0.76, 0.84, 0.93, 1.04, 1.15, 
                   1.28, 1.44, 1.64, 1.96, 2.05, 2.17, 2.33, 2.58)
  )
table5_1 %>% 
...
```{r table5-2, echo = F}
table5_2 <- tibble(
  `Benchmark method` = c("평균", "단순", "계절성 단순", "표류"),
  `h-step forecast standard deviation` = c(
    "$\\hat \\sigma_{h} = \\hat \\sigma \\sqrt{1+1/T}$",
    "$\\hat \\sigma_{h} = \\hat \\sigma \\sqrt{h}$",
    "$\\hat \\sigma_{h} = \\hat \\sigma \\sqrt{k+1}$",
    "$\\hat \\sigma_{h} = \\hat \\sigma \\sqrt{h(1+h/T)}$"
  )
)

table5_2 %>%
  kableExtra::kbl(caption = "Multi-step forecast standard deviation for the four benchmark methods, where \\sigma is the residual standard deviation, m is the seasonal period, and k is the integer part of (h-1)/m (i.e., the number of complete years in the forecast period prior to time T + h).") %>%
  kableExtra::kable_minimal()
```
...
```{r, warning = F, fig.cap = "단순 방법에 기초한 구글 종가에 대한 80%와 95% 예측 구간"}
google_2015 %>%
  model(NAIVE(Close)) %>%
  forecast(h = 10) %>%
...
```{r, warning = F, fig.cap = "부트스트랩 잔차를 이용한 단순 방법에 기초한 구글 종가의 다섯 가지 시뮬레이션된 미래 표본 경로"}
google_2015 %>%
  ggplot(aes(x = day)) +
...
```{r, warning=F, fig.cap = "부트스트랩 잔차를 이용한 단순 방법으로 구한 구글 종가분석에 대한 예측값"}
fc %>%
  autoplot(google_2015) +
  labs(title = "구글 일별 종가", 
       y="미국 달러 ($)", x = "일") 
...
+ $\log(x) = -1, \log(y) = 1$이라고 할 때, 우리가 얻은 구간이 $[-1, 1]$이라고 하자. 원래의 척도로는 약 [`r round(c(exp(-1), exp(1)), 2)`]의 구간이 되어 비대칭적이게 된다.
...
```{r fig5-17, warning = F, fig.cap = "로그값을 취한 데이터에 적용된 표류 방법을 사용한 계란 가격의 예측값. 편향 조정된 평균 예측값은 실선으로, 중앙값은 점선으로 나타난다."}
prices %>%
...
```{r fig5-18, warning=F, fig.cap = "전미 소매 고용에 대한 STL 분해로부터 얻은 계절적으로 조정된 데이터에 대한 단순 예측값(Naïve forecasts)"}
...
```{r fig5-19, warning=F, fig.cap = "데이터를 STL 분해한 이후에 계절적으로 조정된 데이터에 대한 단순 예측값과게절적 요인에 대한  계절성 단순 예측값에 기초한 전미 소매 고용에 관한 예측값"}
...
```{r fig5-20, warning=F, fig.cap = "잔차 확인하기"}
...
knitr::include_graphics("image/traintest-1.png")
```
...
```{r fig5-21, warning=F, fig.cap = "2007년 말까지의 데이터를 사용한 호주의 분기별 맥주 생산량 예측값"}
...
```{r fig5-8, warning=F, fig.cap="2016년 1월에 대한 구글 주가의 예측값"}
...
```{r fig5-23, warning=F, fig.cap = "80% 예측구간으로 구한 2016년 1월 구글 주가의 단순 예측값"}
google_fc %>%
...
```{r, echo = F, out.width="100%"}
knitr::include_graphics("image/cv1-1.svg")
```
...
```{r, echo = F, out.width="100%"}
knitr::include_graphics("image/cv4-1.svg")
```
...
```{r, warning=F, fig,cap = "구글 종가에 적용한 표류 방법에 대한 예측 범위의 함수로서 RMSE"}
google_2015_tr <- google_2015 %>%
  stretch_tsibble(.init = 3, .step = 1)
